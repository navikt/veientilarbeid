...
describe('Test av komponent', () => {
    test('Rendrer komponent hvis skal vises', async () => {
        ...
        expect(
            screen.getByText(/hvis du har søkt eller ønsker å søke dagpenger må du ta kontakt med oss./i)
        ).toBeTruthy();
        expect(screen.getByText(/ring 55 55 33 33 med tastevalg 2./i)).toBeTruthy();
        expect(
            screen.getByText(
                /hvis det ikke er aktuelt å søke dagpenger nå, eller du har aap kan du se bort fra denne meldingen./i
            )
        ).toBeTruthy();
    });
